...
function init(ctx) {
  ...
  function autoSnoozeAlarms(mbgMessage, treatmentMessage, lastTreatment, sbx) {
	...
	//Auggie: Bypass snooze for carb treatments
    const carbs = Number((lastTreatment && lastTreatment.carbs) || 0);
    if (carbs > 0) return;
  ...
  function requestTreatmentNotify (lastTreatment, sbx) {
    ...
    if (lastTreatment.isAnnouncement) {
      requestAnnouncementNotify(lastTreatment, sbx);
		...
		//Auggie: Carb-only filter: skip notifications unless carbs > 0
			const carbs = Number(lastTreatment && lastTreatment.carbs || 0);
    		if (!carbs || carbs <= 0) return;
	  ...
	  //Auggie - critical alerts
	  const alertLevel = sbx.levels.URGENT
      sbx.notifications.requestNotify({
        level: alertLevel
